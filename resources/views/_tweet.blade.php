    ...
    <div class="flex p-4 {{$loop->last?'':'border-b border-b-gray-400'}}">
        ...
        <div>
            ...
            <p class="text-sm justify-content-end mb-3">{{$tweet->body}}</p>
           <div class="flex">
            ...
            <div class="flex items-center mr-4 {{$tweet->isLikedBy(auth()->user())?'text-blue-500':'text-gray-500'}}">
                ...
                <button type="submit" class="text-xs text-gray-500">{{$tweet->likes()->where('liked',true)->count()}}</button>
            </div>
            </form>
            ...
            <div class="flex items-center {{$tweet->isDisLikedBy(auth()->user())?'text-blue-500':'text-gray-500'}}">
                ...
                <button type="submit" class="text-xs text-gray-500">{{$tweet->likes()->where('liked',false)->count()}}</button>
            </div>
               </form>
